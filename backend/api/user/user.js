...
class user {
    ...
    async todoList(ctx) {
        let res;
        let todoList = await mysqlJs.queryFromMysql(`SELECT * FROM todoList`);

        console.log(todoList);
        if (todoList) {
            // res = {
            //     name: todoList[0].name,
            //     checked: todoList[0].checked
            // }
            res =  todoList;
            // console.log(res)
            return ctx.body = res;
    ...
    async addtodoList(ctx) {
        ...
        let is_exist = await mysqlJs.queryFromMysql(`SELECT * FROM todoList WHERE name = '${name}'`);
        if(name===''){
            res = {
                status: 201,
                name: '',
            ...
            // console.log("名字为空"+res.status)
            return ctx.body = res;
        }else if(is_exist && is_exist.length > 0){
            res = {
                status: 202,
                name: '',
                checked: '',
            ...
            // console.log("名字占用"+res.status)
            return ctx.body = res;
        }
        else{
            // console.log("准备插入"+checked)
            let inserttodoList = await mysqlJs.queryFromMysql(`INSERT INTO todoList (name,checked,datatime) VALUES ('${name}','${checked}','${datatime}')`);
            res = {
                status: 200,
                name: name,
                checked: checked,
            ...
            console.log(inserttodoList+res.status)
            return ctx.body = res;
        }
    }
    // deletetodoList
    async checktodoList(ctx) {
        ...
        let todoList = await mysqlJs.queryFromMysql(`update todoList set name = '${name}' where datatime ='${datatime}' `);
        console.log(todoList)
        res = {
            status: 200,
            name: name,
    ...
    // deletetodoList
    async deletetodoList(ctx) {
        let res;
        // console.log(ctx)
        let key = ctx.request.body.key;
        let name = ctx.request.body.list.name;
        console.log(key)
        console.log(name)
        // 刪除操作，name肯定存在，故不用做if判断
        let deletetodoList = await mysqlJs.queryFromMysql(`delete from todoList where name = '${name}' `);
